...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X Z G U histo histo_R metric_histo] = SPC_func(T,Q,TV_QV,rho,K,SNR,rate,maxiter,tol,out,Xtrue)
  ...
  for iter = 1:maxiter
    ...
    obj2 = sum_all(E.^2);
    if ~isempty(Xtrue)
      diff_true = double(Xtrue(:)) - double(X(:));
      mse_true = mean(diff_true.^2);
      rmse_true = sqrt(mse_true);
      rse_true = norm(diff_true) / max(norm(double(Xtrue(:))), eps);
      mae_true = mean(abs(diff_true));
      metric_histo(iter,:) = [mse_true rmse_true rse_true mae_true];
    end
