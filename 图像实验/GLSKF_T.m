...
%% 批量处理图像
results = struct();
summary_data = cell(length(image_names), 6);
...
for idx = 1:length(image_names)
    ...
    % 打印最优结果
    fprintf('\n========================================\n');
    fprintf('最优参数: rho=%d, gamma=%d\n', best_params.rho, best_params.gamma);
    fprintf('最优 PSNR: %.4f dB\n', best_result.psnr);
    fprintf('最优 SSIM: %.4f\n', best_result.ssim);
    fprintf('最优时间: %.2f 秒\n', best_result.time);
    fprintf('========================================\n');
    ...
    summary_data{idx, 6} = best_result.time;
end
...
summary_table = cell2table(summary_data, ...
    'VariableNames', {'图像名称', 'PSNR_dB', 'SSIM', '最佳rho', '最佳gamma', 'Time_sec'});

excel_path = sprintf('%s/实验总结.xlsx', result_dir);
writetable(summary_table, excel_path);

fprintf('\n结果已保存。\n');
fprintf('Excel 文件已包含运行时间列。\n');
